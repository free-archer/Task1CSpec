...
assert_no_path() {
  [ ! -e "$ROOT/$1" ] || fail "unexpected path: $1"
}

assert_contains() {
  file=$1
  text=$2
  grep -F -- "$text" "$ROOT/$file" >/dev/null || fail "$file does not contain: $text"
}
...
assert_contains ".task1cspec/AGENTS.md" "Do not rely on local helper scripts or Python."
assert_contains ".task1cspec/skills/task1cspec/SKILL.md" "Не используй локальные helper-скрипты и не требуй Python."
...
printf '%s\n' "OK: Task1CSpec runs through agent file operations without Python helper scripts."
